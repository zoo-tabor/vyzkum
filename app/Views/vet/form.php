...
<div class="card">
    ...
    <?php if ($vetName !== ''): ?><p class="muted">Veterinar: <?= e($vetName) ?></p><?php endif; ?>
    ...
    <form method="post">
        ...
        <label for="chip_number_vet">Cislo cipu psa (15 cislic)</label>
        ...
        <select id="sample_type" name="sample_type" required>
            ...
            <option value="buccal_swab"<?= $st === 'buccal_swab' ? ' selected' : '' ?>>Bukalni ster</option>
            <option value="blood"<?= $st === 'blood' ? ' selected' : '' ?>>Krevni vzorek</option>
            <option value="other"<?= $st === 'other' ? ' selected' : '' ?>>Jine</option>
        </select>

        <label for="sample_type_other">Jiny typ vzorku (nepovinne)</label>
        ...
        <button type="submit" class="btn btn--primary">Odeslat odber</button>
    </form>
</div>
