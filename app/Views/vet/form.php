...
<section class="panel">
  ...
  <?php if ($sample['vet_submitted_at'] !== null): ?>
    <div class="notice ok">Veterinární část už byla uložena. Odkaz je jednorázový.</div>
  <?php else: ?>
    <?php if ($errors): ?>
    ...
    <form method="post">
      ...
      <div class="grid">
        <div>
          ...
          <div class="field-note">Typicky 15 číslic.</div>
        </div>
        <div>
          ...
          <select id="sample_type" name="sample_type" required>
            ...
            <option value="buccal_swab" <?= $selectedType === 'buccal_swab' ? 'selected' : '' ?>>Bukální stěr</option>
            <option value="blood" <?= $selectedType === 'blood' ? 'selected' : '' ?>>Krevní vzorek</option>
            <option value="other" <?= $selectedType === 'other' ? 'selected' : '' ?>>Jiné</option>
          </select>
        </div>
        <div>
          <label for="sample_type_other">Jiný typ vzorku</label>
      ...
      <div class="actions">
        <button type="submit">Odeslat odběr</button>
      </div>
    </form>
  <?php endif; ?>
</section>
