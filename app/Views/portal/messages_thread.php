...
<div class="page-head">
    <h1><?= e($heading) ?></h1>
    <p><a href="/portal/messages">&larr; <?= t('Zpět na zprávy') ?></a></p>
</div>
...
<div class="card">
    ...
    <?php else: ?>
        <?php foreach ($messages as $m): ?>
            <div style="border-top:1px solid var(--line); padding:0.5rem 0;">
                <strong><?= ($m['sender_role'] ?? '') === 'owner' ? t('Vy') : t('Výzkumný tým') ?></strong>
    ...
    <form method="post" action="/portal/messages" style="margin-top:0.75rem">
        ...
        <label for="body"><?= t('Nová zpráva') ?></label>
        <textarea id="body" name="body" rows="3" required></textarea>
        <button type="submit" class="btn btn--primary"><?= t('Odeslat') ?></button>
    </form>
</div>
