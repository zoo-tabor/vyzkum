...
<?php if ($breeds === []): ?>
    <div class="card"><p>Váš účet zatím nemá přiřazené žádné plemeno.</p></div>
<?php else: ?>
    ...
    <div class="card">
        ...
        <?php else: ?>
            <table class="table">
                <thead><tr><th>Jméno</th><th>Pohlaví</th><th>Narození</th><th>Stav</th><th>Majitel</th></tr></thead>
                <tbody>
            ...
            <p class="muted">Kontaktní údaje majitelů nejsou klubům zpřístupněny.</p>
            <div class="pager">
                <span class="muted"><?= $pager->from() ?>-<?= $pager->to() ?> z <?= $pager->total ?></span>
                <span>
                    <?php if ($pager->hasPrev()): ?><a class="btn" href="/club/dogs?breed=<?= $selected ?>&page=<?= $pager->page - 1 ?>">&larr; Předchozí</a><?php endif; ?>
                    <?php if ($pager->hasNext()): ?><a class="btn" href="/club/dogs?breed=<?= $selected ?>&page=<?= $pager->page + 1 ?>">Další &rarr;</a><?php endif; ?>
                </span>
            </div>
        <?php endif; ?>
    </div>
