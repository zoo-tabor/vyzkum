...
<div class="card">
    <?php if ($batches === []): ?>
        <p class="muted"><?= t('Zatím žádné dávky.') ?></p>
    <?php else: ?>
        <table class="table">
            <thead><tr><th>#</th><th><?= t('Popis') ?></th><th><?= t('Plemeno') ?></th><th><?= t('Veterinář') ?></th><th><?= t('Sad') ?></th><th><?= t('Vytvořeno') ?></th><th></th></tr></thead>
            <tbody>
            <?php foreach ($batches as $b): ?>
                <tr>
                    ...
                    <td><a href="/admin/batches/<?= (int) $b['id'] ?>/labels" target="_blank"><?= t('Tisk štítků') ?></a></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
